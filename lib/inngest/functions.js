...
export const checkBudgetAlerts = inngest.createFunction(
  ...
  async ({ step }) => {
    const budgets = await step.run("fetch-budgets", async () => {
      ...
      // Whatever a step returns is JSON-serialized so it can be replayed on
      // retry: Decimals come back as strings and Dates as ISO strings. Flatten
      // to primitives here, once, rather than guessing about it below.
      return rows.map((budget) => ({
        ...
        defaultAccount: budget.user.accounts[0] ?? null,
      }));
    });

    for (const budget of budgets) {
      if (!budget.defaultAccount) continue;

      await step.run(`check-budget-${budget.id}`, async () => {
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);

        if (budget.amount <= 0) return { skipped: "no budget set" };

        const expenses = await db.transaction.aggregate({
          where: {
            userId: budget.userId,
            accountId: budget.defaultAccount.id,
            type: "EXPENSE",
        ...
        await sendEmail({
          to: budget.email,
          subject: `Budget Alert for ${budget.defaultAccount.name}`,
          react: EmailTemplate({
            userName: budget.userName ?? "there",
            type: "budget-alert",
            data: {
              percentageUsed,
